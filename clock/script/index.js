...
setInterval(() => {
    ...
    document.getElementById("clock").textContent = `${now.getHours() > 9 ? now.getHours() : "0" + now.getHours()}:${now.getMinutes() > 9 ? now.getMinutes() : "0" + now.getMinutes()}:${now.getSeconds() > 9 ? now.getSeconds() : "0" + now.getSeconds()}`;
